...
against, rather than answers generated from the detector:

| directory | what it is | built by |
|---|---|---|
| `corpora/` | the essay corpora and the registry naming which applies; `persuade-20` ships its twenty essays | `tools/persuade_build.py` |
| `census/` | US surname frequencies, all 162,253 rows of the 2010 release, for the bare-surname gate | `tools/census_build.py` |

These two are why every gate is measurable on a bare checkout. Neither is
regenerated by `just sync-conformance` and neither should be: their builders read
a local upstream, pin its digest, and are run by hand when that upstream changes.
...
Four of the nine gates declare a `requires` — three need an essay corpus, one US
surname frequencies. Both are satisfied on a bare checkout: `persuade-20` ships in
`corpora/` and is the registry default, and `census/` ships the surname table.
So a fresh clone reaches all nine, and so does CI.

That does not retire the rule. A runner that cannot reach a gate must report
`NOT MEASURED` **by name** and must not reduce the denominator: eight of nine held
is a different statement from nine of nine, and a badge cannot tell them apart.
Each port's gate suite still measures the four `requires` gates with their inputs
deliberately withheld, and asserts they report NOT MEASURED — the machinery is
what makes the *next* unreachable gate visible, so it is tested whether or not a
shortfall is currently reachable.
