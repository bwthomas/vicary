...
def test_the_interior_channel_reads_the_document_the_word_scan_would_have(
    ...
    looks at :data:`_INTERIOR_CAP_HINT` and walks out from each hit to the word
    carrying it. The hint is a necessary condition for
    :func:`has_interior_capital`, so the answer is unchanged; this pins that
    ...
    is the one that matters — both characters are in :data:`_WORD_TOKEN`'s
    continuation class, so a word-boundary assertion would have skipped the
    token entirely, and ``'ChoaCh'`` hints at its own FIRST letter before it
    hints at the interior one.
    """
