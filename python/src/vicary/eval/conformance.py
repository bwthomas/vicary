    ...
    return {
        ...
        "requirements": {
            ...
            "census": "US surname frequencies. `conformance/census/` ships the "
                      ...
                      "path never reads it.",
            "latency_baseline": "A PAIRED measurement, taken on the machine "
                                "running the gate: tools/latency_pair.py checks "
                                "the previous release out of this repository's "
                                "own history and times it and this checkout "
                                "alternately, then leaves both numbers in a "
                                "record VICARY_LATENCY_PAIR points at. Nothing "
                                "is stored between releases, because a stored "
                                "number assumes the machine is a constant and it "
                                "is not: identical code spread 67% across six "
                                "ubuntu-latest runners in Ruby, 26% in Python "
                                "and 21% in TypeScript, from a runner label that "
                                "served five different CPU models. Measuring "
                                "both sides here cancels all of that. Without "
                                "the record the number is still measured and "
                                "printed — only the verdict is withheld, because "
                                "one side of a comparison is not a gate.",
        },
        "gates": [
            {
                "id": "held_out_recall",
            ...
            },
            {
                ...
                "why": "How much slower this port redacts the corpus than the "
                       "last release does, per implementation, with both "
                       "measured on the same machine minutes apart. Two "
                       "designs preceded this one and each decided a release "
                       "wrongly. An absolute 10 ms bar passed on a laptop and "
                       "failed on the runner enforcing it, so 0.2.3 published "
                       "to PyPI and npm while RubyGems refused the same "
                       "commit. A stored per-release baseline then red-lit "
                       "main on unchanged code at +8.33%, because the profile "
                       "it pinned — ubuntu-latest — is not a machine: 36 "
                       "processes over six runners spread 67% in Ruby, 26% in "
                       "Python and 21% in TypeScript on identical code, and "
                       "the label served five CPU models. Timing the previous "
                       "release HERE cancels every one of those terms, "
                       "leaving within-process noise of 0.7-3.3%. The figure "
                       "is the median of every essay x every repeat pooled — "
                       "100 samples, asset load and a full warmup pass "
                       "excluded — NOT a percentile: at n=20 essays a p95 IS "
                       "the maximum, and a maximum moves on a scheduling "
                       "pause rather than on code. Without a paired record the "
                       "gate reports NOT MEASURED and says why, because one "
                       "side of a comparison is not a gate.",
            },
            {
                "id": "asset_entries",
                "label": "asset entries",
